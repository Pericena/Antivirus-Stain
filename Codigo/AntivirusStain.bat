...
rem attrib -h -r -s  /s /d %USB%\:*.*
rem CMD /K NETSTAT -ANOB -P TCP 04|FINDSTR /C:ESTABLISHED> %userprofile%\Desktop\Informe.txt
rem cmd /k NETSTAT -ANOB -P TCP 04|FINDSTR /C:ESTABLISHED & ping -n 6 127.0.0.1>%userprofile%\Desktop\informe.txt
rem CMD /C WMIC /Output:STDOUT STARTUP get /all /format:LIST>%userprofile%\Desktop\informe.txt
rem CMD /K WMIC /Output:STDOUT STARTUP get /all /format:LIST | more>informe.txt
rem attrib -s -h *.* /s /d
...
echo.             
echo Hola %USERNAME%
echo   %userprofile%                            B@X     BX          
echo   %SYSTEMROOT%                               i@r      @Bs    9s        
echo   %COMPUTERNAME%                            @B@      B@    B@B        
echo   %NUMBER_OF_PROCESSORS%                                        :@B:    r@B    BBi        
echo            @B,    B@                        s@G    ,B@    B@         
